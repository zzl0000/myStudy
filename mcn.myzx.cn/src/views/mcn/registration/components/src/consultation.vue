<template>
  <Row style="padding: 0 20px 40px 20px; line-height: 36px; font-size: 14px">
    <Col span="8"> <span style="font-weight: 600">患者姓名:</span> {{ query.patient_name }} </Col>
    <Col span="4"> <span style="font-weight: 600">性别:</span> {{ query.patient_sex }} </Col>
    <Col span="4"> <span style="font-weight: 600">年龄:</span> {{ query.patient_age }} </Col>
    ...
    <Col span="8"> <span style="font-weight: 600">联系方式:</span> {{ query.patient_mobile }} </Col>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      getConsultation(data.id).then((response) => {
        ...
        console.log(detail.value.order_status);
      });
