  ...
  >
    <BasicForm @register="registerForm" v-show="!done" v-loading="loadingRef">
      ...
      <template #amount="{ model, field }">
        <a-input-number step="0" :min="0.0" v-model:value="model[field]" style="width: 30%" />
      </template>
...
<script setup lang="ts">
  ...
  const formSchema: FormSchema[] = [
    ...
    },
    {
      ...
      componentProps: {
        options: [
          { label: '30分钟', value: '30' },
          { label: '8个小时', value: '480' },
          { label: '24个小时', value: '1440' },
          { label: '72个小时', value: '4320' },
          { label: '永久', value: '525600' },
        ],
      },
      show: ({ values }) => {
  ...
  const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
    ...
    if (unref(isUpdate)) {
      ...
      const initData = {
        ...
        patient_sex: data.record.patient.sex,
        patient_phone: data.record.patient.mobile.split(',')[0],
        patient_age: parseInt(data.record.patient.age),
        doctor_id: data.record.doctor_id,
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (unref(isUpdate)) {
        params.patient_id = datas.value?.patient.paid;
