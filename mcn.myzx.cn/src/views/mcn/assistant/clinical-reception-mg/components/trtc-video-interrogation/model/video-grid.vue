<template>
  <div ref="testRef" id="video_grid" class="video_grid" style="border: none">
    <!-- mr-5 local-stream  -->
    <div id="local_stream" class="remote-stream-p" v-show="isLocalStream">
    ...
    <!-- mr-5 -->
    <div v-for="(item, index) in remoteStreamArr" :key="index" class="remote-stream">
      <!-- doctor医生 -->
      <div
        :class="{ 'remote-stream remote-stream-d': item.view }"
        v-if="item.type == 'doctor'"
        :id="`remote-stream-${item.id}`"
      ></div>
      <!-- patient患者 -->
      <div
        :class="{ 'remote-stream remote-stream-o': item.view }"
      ...
      <!-- <div
        id="local_stream"
        v-html="item.view"
        :class="{ 'remote-stream remote-stream-p': item.view }"
...
<script lang="ts">
  ...
  import { ref, defineComponent, nextTick, watch, computed, onMounted } from 'vue';
  ...
  export default defineComponent({
    ...
    emits: ['handleUpdate', 'checkoutBig'],
    setup(props, { emit }) {
      ...
      onMounted(() => {
        console.log(22222222222, testRef.value);
        // watchEffect(()=>{
      ...
      const addUser = async (userId: any) => {
        ...
        (data[0].type =
          data[0].userID.indexOf('d') != -1
            ? 'doctor'
            : data[0].userID.indexOf('u') != -1
            ? 'patient'
            : 'other'),
          // console.log(data);
          // m = data;
      ...
      const addStream = (stream) => {
        ...
        // console.log(333333333333); data-type="${}"
        ...
        if (!isExist) {
          arr.push({
            ...
            type:
              stream.getUserId().indexOf('d') != -1
                ? 'doctor'
                : stream.getUserId().indexOf('u') != -1
                ? 'patient'
                : 'other',
          });
      ...
      const playStream = async (stream: any) => {
