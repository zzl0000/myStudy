      ...
      <div class="tabs-item">
        ...
        >
          <div class="checkbox-item" :span="24" v-for="(k, i) in historyData" :key="i">
            <div class="prescription-check" style="">
              <a-checkbox :value="k">
                <span >{{ `${k.optionsData.time} 治疗方案` }}</span>
              </a-checkbox>
            </div>
            <div class="prescription-detail" style="width: calc(100% - 175px);">
    ...
    setup(props, { emit }) {
      ...
      const getPrescriptListApiSync = async () => {
        let params = {
          ...
          patient_mobile: state.patient_mobile == ''? props.formState.patient_mobile :state.patient_mobile,
          doctorId: state.doctor_id == ''? props.formState.doctor_id :state.doctor_id,
          // doctorId: state.doctor_id || props.formState.doctor_id,
          ...pageCf,
        };
        try {
